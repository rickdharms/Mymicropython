...
def send_command():
    ...
    try:
        ...
        text = text_var.get()
        color = color_var.get()
        brightness = brightness_var[row].get() / 100  # Brightness from slider
        ...
        command = {
            ...
            "text": text,
            "color": [r, g, b],
            "brightness": brightness,  # Add brightness to the command
        }
...
def reset_inputs():
    ...
    for slider in brightness_sliders:
        slider.set(100)  # Reset brightness to 100%
...
# Brightness Controls
tk.Label(root, text="Brightness:").grid(row=4, column=0, sticky="e")
brightness_sliders = []
brightness_var = []
for i in range(5):  # Create a slider for each row
    var = tk.IntVar(value=100)  # Default brightness is 100%
    slider = tk.Scale(root, from_=0, to=100, orient="horizontal", variable=var)
    slider.grid(row=5 + i, column=1, sticky="we")
    tk.Label(root, text=f"Row {i + 1}:").grid(row=5 + i, column=0, sticky="e")
    brightness_sliders.append(slider)
    brightness_var.append(var)

# Submit Button
tk.Button(root, text="Submit", command=send_command).grid(row=10, column=0, columnspan=2)

# Reset Inputs Button
tk.Button(root, text="Reset Inputs", command=reset_inputs).grid(row=10, column=2)

# Feedback Label
feedback_var = tk.StringVar()
feedback_label = tk.Label(root, textvariable=feedback_var, fg="green")
feedback_label.grid(row=11, column=0, columnspan=3)
